...
top_10_time <- left_join(top_10, time_tags) %>% 
  mutate(pct = n/189) %>% 
  filter(!is.na(frequency)) %>% 
  mutate(frequency = factor(frequency, levels = c("Less than a year", "1-2 years", "3-4 years", "5+ years")))
...
time_vars %>% 
  arrange(-`5+ years`) %>% 
  head() %>% 
  mutate(practice = paste0("practices_", practice)) %>% 
  mutate(practice = ifelse(practice == "practices_inclusion", "practices_all_courses_designed_for_inclusion", practice)) %>% 
  left_join(., branding_labels, by = c("practice" = "tag")) %>% 
  mutate(pct = round((`5+ years`/189)*100)) %>% 
  ggplot(aes(x = reorder(label, pct), y = pct)) +
  geom_col(aes(fill = label)) +
  geom_text(aes(label = paste0(pct, "%")), nudge_y = 0.5, hjust = 0, color = transcend_na) +
  coord_flip() +
  theme_bw() +
  theme(legend.position = "none") +
  labs(x = "",
       subtitle = "In the 2024 survey, Canopy schools answered that the following practices \nwere used 5+ years.",
  ...
  scale_y_continuous(expand = c(0,0), limits = c(0, 28), labels = scales::percent_format(scale = 1))
...
#### Update for 8/9
```{r}
dis_agg_all <- dis_agg_all %>% 
  filter(frequency == 1) %>% 
  group_by(practice) %>% 
  summarise(n = n()) %>% 
  arrange(desc(n)) %>% 
  head(12) %>% 
  mutate(variable = paste0("practices_", practice)) %>% 
  left_join(., branding_labels, by = c("variable" = "tag"))

dis_agg_all %>% 
  mutate(label = ifelse(practice == "inclusion", "inclusion", label),
         pct = round(n/189*100)) %>% 
  ggplot(., aes(x = reorder(label, pct), y = pct)) +
  geom_col(aes(fill = label)) +
  geom_text(aes(label = paste0(pct, "%")), nudge_y = 0.5, hjust = 0, color = transcend_na, fontface = "bold", size = 3, family = "sans") +
  coord_flip() +
  theme_bw() +
  ...
  theme(legend.position = "none",
        panel.grid.major.y = element_blank()) +
  labs(x = "Top Practices",
       y = "Percentage of Schools Selecting",
       title = "Practices Most Frequently Selected by Canopy schools in 2024") +
#  scale_fill_manual(values = c(transcend_cols2[2], transcend_na, transcend_cols[1])) +
  theme(
    plot.title = element_text(family = "Bebas Neue", color = "black"),
    plot.background = element_blank(),
    axis.text = element_text(colour = "black"),
...
dis_agg_all %>% 
  ...
  geom_text(aes(label = paste0(pct, "%")), nudge_y = 0.5, hjust = 0, color = transcend_na, fontface = "bold", size = 4, family = "sans") +
  coord_flip() +
  theme_bw() +
  scale_y_continuous(expand = c(0,0),
                     limits = c(0, 100),
...
### Models: see disagtag_modeling.Rmd
