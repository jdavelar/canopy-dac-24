...
transformative_factors <- bind_rows(local_funds, private_funds, donations, inflation, enrollment, staffing, fiscal_cliff) %>% 
  ...
  # mutate(fill_color = ifelse(p.value >= 0.5, transcend_na, 
  #                       ifelse(exp_est < 1, scales::gradient_n_pal(c(transcend_cols[2], "white"))(exp_est / min(exp_est)),
  #                              scales::gradient_n_pal(c("white", transcend_cols[1]))((exp_est - 1) / (max(exp_est) - 1)))))
#rm(local_funds, private_funds, donations, inflation, enrollment, staffing, fiscal_cliff)
#create heatmap
ggplot(transformative_factors, aes(factor, term, fill = exp_est)) + #fill = fill_color
  geom_tile() +
  scale_fill_gradient(low = transcend_cols[3], high = transcend_cols[1]) +
  #scale_fill_identity() +
  scale_x_discrete(expand = c(0,0), labels = wrap_format(13)) +
  labs(title = str_wrap("School characteristics predicting future factors of concern", 60),
       subtitle = str_wrap("Darker blue indicates that characteristic made a school more likely to select the factor as something that will be transformative in the next 5 years, and a lighter blue made a school less likely to select that factor.", 80),
       x = "",
       y = "") +
  theme(legend.position = "none",
...
variables %>% 
  ...
  labs(x = "",
       y = "Number of schools",
       title = str_wrap("Factors Canopy school leaders think will be most transformative in the next 5 years", 60))
```
...
#plot
ggplot(transformative_factors, aes(factor, reorder(term, exp_est), fill = exp_est)) +
  geom_tile() +
  scale_fill_gradient(low = transcend_cols[4], high = transcend_cols[1]) +
  scale_x_discrete(expand = c(0,0)) +
  labs(title = "School characteristics predicting future transformative factors",
       subtitle = str_wrap("Darker blue indicates that characteristic made a school more likely to select the factor as something that will be transformative in the next 5 years, and a lighter blue made a school less likely to select that factor.", 80),
       x = "",
       y = "") +
  theme(legend.position = "none")
