...
ggplot(transformative_factors, aes(factor, term, fill = fill_color)) + #fill = exp_est
  ...
  labs(title = str_wrap("School characteristics predicting future concern", 55),
       subtitle = str_wrap("Blue indicates a characteristic that made a school more likely to select the factor as something that will be transformative in the next 5 years and red indicates a characteristic associated with less likelihood to select that factor. Gray indicates a characteristic that was not statistically significant.", 73),
       x = "",
       y = "") +
  theme(legend.position = "none",
...
transformative_factors <- bind_rows(ai, mental_health, workforce) %>% 
  mutate(log_exp_est = log(exp_est),
         fill_color = ifelse(p.value >= 0.5, transcend_na,
                        ifelse(log_exp_est < 0, scales::gradient_n_pal(c(transcend_cols[3], "white"))((log_exp_est - min(log_exp_est)) / abs(min(log_exp_est))),
                               scales::gradient_n_pal(c("white", transcend_cols[1]))((log_exp_est - 0) / (max(log_exp_est) - 0))))) %>% 
  filter(fill_color != transcend_na) %>% 
  group_by(factor) %>% 
  arrange(-log_exp_est, .by_group = TRUE) %>% 
...
#save plot
  png(filename = "output/janette-images/figure_10.png",
    ...
    res = 300)
#plot
ggplot(transformative_factors, aes(factor, term, fill = fill_color)) +
  geom_tile(width = 0.8) +
  scale_fill_identity() +
  scale_x_discrete(expand = c(0,0)) +
  labs(title = str_wrap("School characteristics predicting selection of future transformative factors", 50),
       subtitle = str_wrap("Blue indicates a characteristic that made a school more likely to select the factor as something that will be transformative in the next 5 years and red indicates a characteristic that made a school less likely to select that factor, with darker colors representing a stronger relationship.", 75),
       x = "",
       y = "") +
  theme(legend.position = "none",
        panel.grid.major = element_blank(),
