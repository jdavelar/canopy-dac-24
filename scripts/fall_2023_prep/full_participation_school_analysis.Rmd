...
Modify categories above to incorporate changes for BIPOC and repeated participation
...
Mapping priority schools to their respective nominators

Updates: The code below matches schools to nominators, but there's a couple of issues that need to be resolved before finalizing/exporting:  
1. 79 schools are missing a nominating org. A quick check in AT shows that some of these schools may have been in previous batch nominations (NSVF showed up right away e.g., but for 2019 as well as 2022). I also observed a case where the nomination has been rescinded.
2. Some schools have many nominators, so while there are 341 schools to manage as priority schools, that number balloons up to 642 associated nominators. This might not be a big issue however, if we elect for blanket messaging.
