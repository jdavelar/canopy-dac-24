...
top_covid_waffle_tags = waffle_display |>
  slice_max(n_covid_waffle, n = 10) |>
  pull(tag)

top_noncovid_waffle_tags = waffle_display |>
  slice_max(n_reg_waffle, n = 10) |>
  pull(tag)
...
ggplot(
  ...
  labs(x = "Number of Bumps and Dips", y = "", title = "Most waffled tags (non-COVID)") +
  scale_y_tag() +
...
ggplot(
  ...
  geom_col() +
  facet_wrap(vars(COVID), ncol = 1) +
  labs(x = "Number of Bumps and Dips", y = "", title = "Most waffled tags (COVID)") +
  scale_y_tag() +
  theme(panel.grid.major.y = element_blank())
```
