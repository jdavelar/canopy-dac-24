...
waffle_all = long_4y |>
  ...
  filter(
    ...
    n() > 2,                ## at least 3 survey respones
    .by = c(var, school_id)
  ) |> 
  mutate(
    added = any(diff(usage) == 1),
  ...
  mutate(
    ...
    covid_bump = 1 %in% `2021` & waffled,
    covid_dip =  0 %in% `2021` & waffled
  ) 

waffle_all |>
  count(binary_summary, waffled, covid_bump, covid_dip) |>
  arrange(desc(n)) |>
  datatable(
    rownames = FALSE,
    options = list(autoWidth = TRUE, iDisplayLength = 25),
    caption = "Pattern binary summaries"
  )

waffle_all |>
  count(tag, binary_summary, waffled, covid_bump, covid_dip) |>
  arrange(desc(n), tag) |>
  datatable(
    rownames = FALSE,
    options = list(autoWidth = TRUE, iDisplayLength = 25),
    caption = "Tag binary summaries"
  )

waffle_all |>
  count(school_id, school_name, binary_summary, waffled, covid_bump, covid_dip) |>
  arrange(desc(n), school_name) |>
  datatable(
    rownames = FALSE,
    options = list(autoWidth = TRUE, iDisplayLength = 25),
    caption = "School binary summaries"
  )
