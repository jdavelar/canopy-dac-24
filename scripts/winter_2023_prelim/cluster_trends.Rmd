...
pacman::p_load(tidyverse, here, rio, DT, parameters, ggrepel, plotly)
#read in data
...
prep_overall <- function(data, cluster, group){
  dat <- data %>% 
    filter(var %in% cluster) %>% 
...
```{r}
deep <- prep_overall(dat, deeper, repeaters_2)
#cluster_plot_across(deep, "year", "pct", "label")
fancy_across(deep)
```
...
```{r}
deep <- prep_overall(dat, deeper, repeaters_3)
fancy_across(deep)
```
...
```{r}
justice_dat <- prep_overall(dat, ed_justice, repeaters_2)
fancy_across(justice_dat)
```
...
```{r}
justice_dat <- prep_overall(dat, ed_justice, repeaters_3)
fancy_across(justice_dat)
```
...
```{r}
ind_dat <- prep_overall(dat, individualized, repeaters_2)
fancy_across(ind_dat)
```
...
```{r}
ind_dat <- prep_overall(dat, individualized, repeaters_3)
fancy_across(ind_dat)
```
...
```{r}
post_dat <- prep_overall(dat, postsecondary, repeaters_2)
fancy_across(post_dat)
```
...
```{r}
post_dat <- prep_overall(dat, postsecondary, repeaters_3)
fancy_across(post_dat)
```
...
```{r}
none_dat <- prep_overall(dat, none, repeaters_2)
fancy_across(none_dat)
```
...
```{r}
none_dat <- prep_overall(dat, none, repeaters_3)
fancy_across(none_dat)
```
...
```{r}
blend_dat <- prep_overall(dat, blended_tags, repeaters_2)
fancy_across(blend_dat)
```
...
```{r}
blend_dat <- prep_overall(dat, blended_tags, repeaters_3)
fancy_across(blend_dat)
```
