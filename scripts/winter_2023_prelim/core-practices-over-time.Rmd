...
implementation_with_color <- left_join(implementation_time_plot, old_clusters, by = "practice") %>% 
  mutate(
...
implementation_repeaters_with_color <- left_join(implementation_time_repeaters, old_clusters, by = "practice") %>% 
  mutate(
    color = cluster_colors[cluster],
    practice = fct_inorder(glue("<i style='color:{color}'>{practice}</i>"))
  )
