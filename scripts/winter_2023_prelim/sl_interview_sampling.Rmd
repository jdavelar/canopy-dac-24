...
vars <- import(here("data/longitudinal", "combined-dat-ip.csv")) %>% 
  select(school_id, year = panel_year, grades_elementary, grades_middle, grades_high, locale, school_descriptor) %>% 
  unique() %>% 
  ...
  select(school_id, year, type, level, locale) %>% 
  pivot_wider(names_from = "year",
              values_from = c("type", "level", "locale")) %>% 
  ...
  select(school_id, type = merge_type, locale = merge_locale, level = merge_level)
#merge with schools
above_avg_all %>% 
  bind_rows(above_avg_ltd) %>% 
  select(school_name) %>% 
